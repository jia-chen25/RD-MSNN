...
def gov_eqn(f_u, x, params):
    ...
    u_val = u[:, 0:1]  # (N, 1)
    # === 计算源项 f(x, y) === 基于解析解 u_exact = sin(a1 π x) * sin(a2 π y)
    x_coord = x[:, 0:1]
    y_coord = x[:, 1:2]
...
NN_loss = loss_create(pred_u, cond, params_eqn, lw, loss_ref=1)  # loss_ref = 1对损失函数进行放缩
...
f_psi1 = lambda z: pred_u(trained_params, z)  # 创建训练后的解函数
num_points = 1000
...
loss_all = loss_nn1
save_numzeros = []
with open("pde_numzeros.txt", "w", encoding="utf-8") as f:
    f.write("x方向的零点总数记录\n")
    for i in range(len(R_grid)):
        row_data = f1_p.reshape(R_grid.shape)[i, :]
    ...
    for j in range(len(R_grid)):
        col_data = f1_p.reshape(R_grid.shape)[:, j]
...
pred_u2 = mNN_pred_create(f_psi1, lmt, scl2, epsil, act_s=1)  # 创建第二阶段解函数（基于第一阶段解函数f_u1）
...
f_psi2 = lambda z: pred_u2(trained_params2, z)  # 创建训练后的解函数
f2_p = gov_eqn(f_psi2, z_test, params_eqn)
print(jnp.mean(f2_p))

loss_all = loss_all + loss_nn2
pic_dir = os.path.join(os.path.dirname(model_dir), "pic")
plot_loss(loss_all, save_dir=pic_dir, fname="training_losses.png")
save_loss_curve_to_master(loss_all, n_stage=2)
print("Training completed. All outputs saved to:", os.path.dirname(log_file_path))
